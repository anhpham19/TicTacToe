...
public class TTTModel
{
	...
	private final int WIN_LENGTH = BOARD_SIZE;
	...
	private boolean checkVerticalWin()
	{
		...
		{
			//First row can't be compared, so set it as comparison
	...
	/**
	 * BONUS PART 3: Check the diagonals for a win
	...
	private boolean checkDiagonalWin()
	{
		...
		for(row = 0, col = 0; row<BOARD_SIZE; row++, col++)
		{
			...
			for( row = 1, col = 1; col<BOARD_SIZE ; row++, col++)
			{
			    //if current value is same as the previous value, increase the # of connected
				if(gameBoard[row%3][col%3] != 0 && gameBoard[row%3][col%3] == prevValue)
				{
					connected++;
		...
		for (row = 0, col = BOARD_SIZE -1; col >= 0; row++, col--)
		{
			prevValue = gameBoard [row][col];
			for( row = 1, col = BOARD_SIZE - 2; col >= 0 ; row++, col--)
			{
			    //if current value is same as the previous value, increase the # of connected
				if(gameBoard[row][col] != 0 && gameBoard[row][col] == prevValue)
				{
